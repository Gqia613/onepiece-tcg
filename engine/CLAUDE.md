...
実装後は `node tools/lint-fx.js [セット]` を実行し **ERROR 0** を完了条件とする（存在しないop名/フック名=ERROR・optional/once/リーダー対象漏れ疑い=WARN→1件ずつトリアージ）。fx発火実績の診断は `node tools/fx-fire-coverage.js`（デバッグ用・ゲートではない）。パターン網羅チェックリストの全文は `.claude/skills/opcg-card-implement/SKILL.md`。
