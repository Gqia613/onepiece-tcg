...
- **トリガーの空撃ち抑止**（2026-07-23 実対戦報告・P-088ロー）: 「全てcond包み・全check不成立」のトリガーは発動しても何も起こらずトラッシュへ行くだけ＝`triggerDead` が判定し、人間には発動UIを出さず・既定CPUも発動せず手札へ（§11-13）。例外: 場に `onTrigger` リスナー（OP05-109/OP13-106）が居る場合は発動宣言自体に意味があるため従来どおり選択。プロンプト文言も「不発なら手札に加わります」（虚偽）→「発動したカードは場に出ない限りトラッシュへ」に修正。回帰は unit-example 例3g（Q&A841境界含む6assert）。
