...
- **AI: 既定 CPU = heuristic（不変）。「強いCPU/AIモード」= puct 探索（opt-in・デッキ選択画面のトグル）**。`PUCT_DEEP {det:9,look:2,width:8}` を lucy/ace/nami/hancock/teach（+E60: `_OP12-040`クザン/`_OP16-079`ヤマト）に適用（対 heuristic 平均 +19.7pt）。enel は puct/hybrid とも不適合 → `PUCT_MCTS={enel:1}`（mcts 直行）＋`HYBRID_SKIP={enel:1}`。**E60: mihawk/青緑ルフィ/黄キッドは `PUCT_SKIP`＝heuristicフォールバック**（全探索構成で puct ≤ heuristic の実測による）。`STAGE_PLAY={teach:1}`（heuristic の STAGE 設置・teach のみ既定）。`DON_DIFF_W={teach:0.15,hancock:0.15}`。`ai-weights.js` は null（手作り eval 既定）。`ai-policy.js` は puct の prior 用合成方策を出荷。
...
- **E60（2026-08-07・✅SKIP3/DEEP2採用）**: 直近20日の実対戦分析（`docs/pm/cpu-strength-plan-20260807.md`）→ 実利用リーダーのpuct適合性を実測。**mihawk/青緑ルフィ/黄キッドは全構成で puct ≤ heuristic（E53〜59の強化で逆転・mihawkミラー-35pt★）→ `PUCT_SKIP` 採用**。kuzan/yamato は `PUCT_DEPTH` に追加（svbがミラー2帯＋対yamatoクロスで正/中立）。赤緑ルフィのみ基準puctが正＝現状維持（deepはクロス逆符号で保留）。採用バーに「実利用対面クロス非退行」を追加。同日、**cpu_matches 誤終局記録バグ**（web reactAdapter の log/flog に `_sim` ガード漏れ→探索中の一時 G.winner を誤検知・39行中38行が破損）を修正＝対人間勝率が今後実測可能に。詳細 experiments.md E60。
