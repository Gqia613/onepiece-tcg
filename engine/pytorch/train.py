...
VH = int(os.environ.get('AZ_VH', 32))       # value 隠れユニット(旧・1層互換)
VHS = [int(x) for x in os.environ.get('AZ_VHS', '64,32').split(',') if x.strip()]  # ★value 深いNN(隠れ層リスト・既定2層)
VBATCH = int(os.environ.get('AZ_VBATCH', 256)); VPAT = int(os.environ.get('AZ_VPAT', 30))  # ミニバッチ/early stopping辛抱
USE_BOARD = os.environ.get('AZ_BOARD', '') == '1'   # ★1=生盤面boardTensor(336次元)を入力に(17次元evalFeaturesでなく)
...
class MLP(nn.Module):
    """任意層数 MLP。hs(int or list)=隠れ層→各層ReLU→出力1。JS mlpForward(layers)と同一構造。"""
...
def export_mlp(net, mean, std, typ):
    # ★任意層数 → layers:[{W:[out][in], b:[out]}]（JS mlpForward(layers)が読む）。1層も layers 1要素で表現。
    layers = [{'W': [[r5(v) for v in row] for row in l.weight.detach().cpu().tolist()],
               'b': [r5(v) for v in l.bias.detach().cpu().tolist()]} for l in net.layers]
    return {'type': typ,
...
def train_value(rows):
    if len(rows) < 300:
        return None
    KEY = 'bf' if (USE_BOARD and rows and rows[0].get('bf')) else 'f'   # ★生盤面(336) or 17次元
    X = torch.tensor([r[KEY] for r in rows], dtype=torch.float32)
    ...
    net = MLP(X.shape[1], VHS).to(DEV)                       # ★深いNN(VHS=隠れ層リスト)
    opt = torch.optim.Adam(net.parameters(), lr=3e-3, weight_decay=1e-4)
    lossf = nn.BCEWithLogitsLoss()
    ntr = Xtr_s.shape[0]
    import copy as _copy
    best_acc, best_state, patience = 0.0, None, 0
    for ep in range(EPOCHS):                                  # ★ミニバッチ＋early stopping(深いNNの過適合防止)
        net.train()
        idx = torch.randperm(ntr, generator=torch.Generator().manual_seed(100 + ep))
        for bi in range(0, ntr, VBATCH):
            sel = idx[bi:bi + VBATCH]
            opt.zero_grad(); loss = lossf(net(Xtr_s[sel]), ytr[sel]); loss.backward(); opt.step()
        net.eval()
        with torch.no_grad():
            acc = ((torch.sigmoid(net(Xva_s)) >= 0.5).float() == yva).float().mean().item()
        if acc > best_acc + 1e-4: best_acc, best_state, patience = acc, _copy.deepcopy(net.state_dict()), 0
        else:
            patience += 1
            if patience >= VPAT: break
    if best_state: net.load_state_dict(best_state)
    net.eval()
    return export_mlp(net, mean, std, 'mlp'), round(best_acc, 4), n
...
if not POLICY_ONLY and len(VAL) >= 300:
    ...
    weights = {'features': EVALF, 'leaderKeys': LK, 'byLeader': vby, 'default': vdef,
               'inputType': 'board' if USE_BOARD else 'eval',
               'meta': {'samples': len(VAL), 'perLeader': vrep, 'defaultAcc': vacc, 'hidden': (VHS if USE_BOARD else VH), 'input': ('board336' if USE_BOARD else 'eval17'), 'src': 'pytorch/train.py(MPS)'}}
