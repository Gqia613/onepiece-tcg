...
fetch = globalThis.fetch;   // ★stubs.jsの拒否fetchを実fetchに戻す＝proxyへ実際に問い合わせる
G.aiOn = false;
...
(async () => {
  ...
  console.log('CACHE ' + JSON.stringify(LLM_CACHE));
  process.exit(0);
})();
`;

const out = runHarness('warm', harness, { timeout: 590000 });
const m = out.match(/CACHE (\{[\s\S]*\})\s*$/);
if (!m) { console.error('✗ キャッシュ取得失敗\n' + out); process.exit(1); }
const merged = JSON.parse(m[1]);
