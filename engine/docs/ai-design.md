...
### 9.8 Phase2 part6：enel を徹底診断 → ✗ 探索はコントロール/ランプ専用機に勝てない（フォールバック確定）
puct-enel が弱い原因を **行動集計で診断**（enelミラー me=enel vs heuristic）:
| エージェント | 勝率 | atk/turn | ramp/turn | 残ドン/turn |
|---|---|---|---|---|
| heuristic-enel | 50% | 2.18 | 0.83 | **0.84**（カウンター用に温存） |
| puct-enel | **6%** | 2.51（多い） | 0.71（少ない） | **0.27**（使い切る） |
- **診断＝puctはenel(コントロール/ランプ)を「アグロ」に打つ**: ドンをアタックに使い切り、相手ターンのカウンター/受けに温存しない＋ランプも減る。enelの勝ち筋（ドン温存・ランプ・受け）と逆。原因は **境界価値(look=1)が短期の盤面/打点を好む**＝アグロ寄り。
- 試した修正（すべて✗）: **ドン温存reserve=2**（アタックで2ドン未満になる手を除外）→ -25pt（-29ptから僅か改善のみ）／ランプ確定実行・look=3（part5）も✗。
  → **複合的なズレ（少ランプ＋過アグロ＋価値のアグロ嗜好）で単一修正では直らない**。enelを直す＝heuristicのenel制御ロジックを探索内に再実装する事に等しく、それは**フォールバックそのもの**。
- **結論（重要な一般知見）**: **探索/AlphaZeroは“テンポ/アグロ柔軟”なデッキ(lucy/teach/ace/nami/hancock)には+20〜45ptで効くが、“専用コントロール/ランプエンジン”(enel)には効かない**＝この規模では**手調整の戦略知識が生成的探索に勝つ**。`PUCT_SKIP={enel:1}` のフォールバックが正しい安全解（enel=heuristic=50%）。試した修正はすべて撤回。

### 9.9 ★最終到達点（AI開発の結論）
- **強いCPU = puct（本物の探索＋自己対戦policy＋per-leader gating＋replay buffer）。5/6リーダーで heuristic を有意に +20〜45pt 上回る**。UIの「CPU強さ→強い(AI探索)」で有効化（§9.7）。enelのみ探索不適でheuristicフォールバック。
...
### 9.12 探索深さのスケーリングを全リーダーへ採用（2026-07-04・E34）— 強い/AIモード既定を det9/look2/w8 に
ユーザー要望「あらゆる盤面を計算し・相手の動きを読み・先を読む最強AIモード」を受け、§9.3で実証済みの「深さでスケールする」を
全リーダーに展開。ミラー深さプローブ（`tools/puct-depth-probe.js`・N=60/リーダー・同一seedペア・seed帯800000）で4段を測定:
| リーダー | base(3/1/5)対h | det6/look2/w6 | **det9/look2/w8** | det12/look2/w10 |
|---|---|---|---|---|
| lucy | +10.0 | +18.3 | **+30.0 (p=0.000★)** | +28.3 |
| teach | +20.0 | +25.0 | **+25.0 (p=0.001★)** | +20.0 |
| hancock | +13.3 | +15.0 | **+18.3 (p=0.035★)** | +23.3 |
| nami | ±0.0 | +11.7 | **+13.3** | +5.0 |
| ace | +3.3 | +6.7 | **+11.7** | +10.0 |
- **det9/look2/w8 が頂点**（対base合算 改善60/退行29 p≈0.002★・対h平均 +9.7→+19.7pt）。**det12/w10 は4/5リーダーで飽和/微減＝逓減開始**（この先は §10.3 のとおり価値NN葉が要る）。
- **確認測定（新seed帯900000・N=60・選択バイアス排除）**: ace +30.0(p=0.000★)/teach +30.0(p=0.000★)/hancock +16.7(p=0.052)/lucy +21.7(対base+1.7)/nami +3.3(±0)＝**退行リーダーなし・2帯合算 deep対base 改善121/退行59 p<0.00001★**。
- 採用: `src/70-ai.js` `PUCT_DEEP={det:9,look:2,width:8}` を lucy/ace/nami/hancock/teach の `PUCT_DEPTH` に設定（enelは従来どおり det6/2/6→`PUCT_MCTS`でmcts読み・未知リーダーは標準 det3/1/5 のまま）。既定CPU=heuristic は不変・「強い」「AI」モードだけ深くなる。
- 速度: 探索は軽く headless で deep 1局~3s＝1手 <0.5s（🤖思考中バッジ内・UI許容）。基準腕の勝敗が別実行で完全一致＝決定的測定の健全性も確認。
- 学び: ①深さは det6 で止まらず **det9 が単機の最適点**（逓減点は det12）②「同一seed帯で複数tierから最良を選ぶ」選択バイアスは**新seed帯の確認測定で必ず裏取り**（本件は2帯とも合算有意で確定）。

### 9.13 E35: enel単独ローカルAlphaZero反復（2026-07-06）— ❌10世代全棄却＝enel学習ルートを最終確定でclose
クラウドAlphaZeroのフル導入見送り（成果物が6デッキ限定で任意デッキへ汎化しない・汎化版は未解決研究）を受け、
**最後の未検証条件＝「enel単独へのデータ全振り×value+policy共同反復」**をローカルで実測した。
- 実装: `selfplay-puct.js OPCG_TARGET=both`（value=生盤面boardTensor336＋policyを同一世代で共同学習・gatingは学習アーム対hフリップ=measure-matchupのimpL/regL新設）・`OPCG_NOSKIP=1`（enelのPUCT_MCTS代替を無効化し本物のpuctで自己対戦/測定）。
  付随して**橋の潜在バグ2件を修正**: ①`mlpLogit`がtrain.py(part7以降)のlayers形式を読めない（＝policy再学習が全てクラッシュ・E35着手で発覚）②self-playチャンク40局が590sタイムアウト超過で無言死（2晩連続の死因・10局/チャンクに縮小＋1回リトライ＋タイムスタンプ進捗ログ）。
- 測定（enelミラー400局/世代×10世代・det6/look2/w6・replay20000・N=60固定seedペア）: ベースライン=手作りvalue+StageB policyのpuct-enel対h **net=-7**。候補は**10世代すべて棄却**:
  net = -23, -20, -21, -17, -11, -13, -9, -21, -16, -16。世代5-7の上昇は世代8で-21へ逆戻り＝**候補分散（-9〜-23・平均≒-17）でトレンドなし**。
- **学び**: ①gating全棄却→self-play分布が不変→「同一分布のデータ蓄積」に退化し、buf飽和後は世代間差＝学習の乱数だけになる（gated反復の構造的限界。gatingを外す=分布を学習netに任せる変種はpart7で発散済み）。
  ②enel特化・データ集中10倍という**最有利条件でも学習valueは手作りに届かない**＝part3/part5/part7に続く4度目にして最終の✗。
- **結論: enelは「探索も学習も効かない」が最終確定。現行の mcts 代替（PUCT_MCTS・対h+8.3pt）が最適解**。単機の残レバーは Hybrid live実測（§10.4）と lucy/ace/nami の evalState 資源項目のみ。
