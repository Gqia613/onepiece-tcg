...
## E55 / 2026-07-20: リプレイ研究（7/19 tikumaru 5連勝）由来の13部品 → ✅8採用・❌5不採用（S級3部品は要再設計の知見）

- 背景: `docs/pm/replay-study-20260719.md` の候補16件のうち、調査で3件は既存実装/フロー自動成立と判明（盤面フル置換=heuristicTurnの+1閾値入替・バフ先出し=プレイ→アタック順で自動・高ライフ手札温存=現行mode選択で充足）。残り13部品を `E55_DEF`＋e55On で実装（攻撃系8=50-input-cpu-ai.js／防御系4=30-flow-battle.js／効果系1=20-targeting-fx.js）。あわせて resume 経路の入力配達過渡窓バグを inflight-apply ゲートで修正（m12リプレイ246/246完走で実証・コミット1376246）。
- 測定（同一seedペア・2帯・teach↔enel両視点+mihawk→luffygb・N=120・単離52本+合成4本）:
  - ✅採用8: **survblock**(生存ブロック Σ+1.7pt 改善12/退行10=発火最多)・**restcombo**(ブロッカー無力化コンボ Σ+0.7pt)・**pump/trade/refund/actatk/chip/redirect**(全ライン±0=プリセット対面に該当カード・局面が無く不発＝無害。リプレイ/観察で実証済みの対人間部品としてkohand型基準で採用)。合成=Σ+0.8pt 改善13/退行12＝負の相互作用なし。
  - ❌不採用5: **order**(詰め上乗せ順序制御 Σ-9.2pt 3/14・b1 mihawk -5.8pt p=0.016★)＝素で殴らせた中間アタックが相手ブロッカーに生存ブロックされ丸損。「ブロックを吸わせる」には中間打点がブロッカーを倒せる強さが必要＝上乗せ抑止でなく配分最適化として再設計が必要／**alloc**(ターン列防御割当 Σ-8.3pt 3/13)＝受け過ぎで押し切られる列が発生。相手の次ターン打点予測と結合しないと危険／**reko**(再KOチェック Σ-3.3pt 0/4)＝防衛全廃は行き過ぎ。「防衛コスト2枚以上の時だけskip」等に絞って再挑戦／**chase**(-0.8)＝現行スコアで既に追撃可能・不要／**knownlife**(-0.8)＝niche。
...
- **追補（2026-07-23・E59横展開）**: **pumpをpuct候補手にミラー**（candidateActions＝ポンプ持ち相手へのドン沈めリーダー攻撃を候補除外。applyActionの付与は常に最小同値＝ポンプ超え予算を組めないため除外が忠実なミラー。素need0は許可＝ポンプドンを吸える）。webのCPU戦はpuct固定（E56）なのにpumpはcpuPickAttack側のみ＝**ユーザーの紫カタクリデッキ（L OP11-062/OP11-073が該当・全該当8枚中2枚）相手にCPUがポンプ圏内へドンを沈める実害があった**。プリセットは該当カード0＝メーター構造的無風（E55本測定と同じ）なので、E55と同基準（無害＋対人間実証）で採用。回帰: unit-example 例3s（除外/cond不成立で許可/素アタック許可の3assert）。他の採用7部品はpuct側の対応不要と整理: survblock/redirect=フロー側決定で既に有効・restcombo/chip/refund/actatk=探索が順序と手を自前決定（ロールアウト内はcpuShouldPlayEvent経由で有効）・trade=評価バイアスでE41の学びから見送り。
...
## E58 / 2026-07-23: STAGE_PLAY mihawk採用＋actgateコスト支払い判定拡張 → ✅採用（2帯符号再現）

- 背景: 意味照合キャンペーンの残フォロー2件。①mihawkデッキの棺船OP14-039（1ドン=ミホークLで1ドロー＋毎ターン終了時ドン1アクティブ）をCPUが置かない（STAGE_PLAY={teach}のみ・E46のper-leader採用方式）②E57残課題「起動メイン側のrevealCost支払い不能起動」（OP14-105ゴルゴン三姉妹=手札の九蛇/アマゾン・リリー3枚公開。revealCostは原子的に不発だが `_actTurn` は消費済＝【ターン1回】の空費）。
...
## E60 / 2026-08-07: 実利用リーダーのpuct適合性（深さ適用漏れ×heuristic逆転） → ✅SKIP3・DEEP2採用・保留1

- 背景: 直近20日の本番対戦履歴分析（`docs/pm/cpu-strength-plan-20260807.md`）で、ユーザーがCPU側に選ぶデッキの**59%（23/39戦）が PUCT_DEPTH 未適用リーダー**（既定 det3/1/5）と判明。E34深さレバーの適用漏れ仮説で per-leader 測定。同日に cpu_matches の誤終局記録バグ（reactAdapter log/flog の `_sim` ガード漏れ→探索中の一時 G.winner を cpuRecorder が誤検知＝39行中38行が winner='draw'・inputs途中切断で破損）を修正・デプロイ済み（7130ed8）＝以後は対人間勝率が実測可能。
- 測定: `tools/puct-depth-probe.js`（ミラー・N=40×2帯 800000/910000・構成 6/2/6 と 9/2/8・同一seedペア＋符号検定）＋**対yamatoクロス1帯**（実利用最多対面＝人間側の67%が黒ヤマト）。プリセットの無い赤緑ルフィ/黄キッドは実対戦リプレイから抽出した実デッキで測定（`tools/probe-decks.json`＋`tools/puct-depth-probe-custom.js` 新設）。
- 結果（対h効果・svb=strong vs base フリップ）:
  - **mihawk**: 基準 **-35.0★/-10.0**、6/2/6 -25.0/-10.0、9/2/8 -17.5/-12.5。対yamatoクロスも基準 -22.5（改善0/退行9）＝**どの探索構成もheuristicに届かない**
  - **luffygb**: 基準 **-25.0★/-10.0**、深化しても8本中7本で負〜中立（9/2/8帯2のみ+2.5）。クロス±0
  - **黄キッド**（実デッキ）: 基準 -17.5/-15.0、deepでも -7.5〜±0。クロス -7.5/-10
  - **kuzan**: 基準 +27.5★/+17.5、9/2/8 **+35.0★/+30.0★**・svb +7.5/+12.5・クロスsvb **+17.5**（基準の対yamato負をdeepが正に修復）
  - **yamato**: 基準 -10.0/±0、9/2/8 +5.0/+10.0・svb **+15/+10**・クロス（対kidd実デッキ）svb ±0
  - **赤緑ルフィ**（実デッキ）: 基準 **+12.5/+7.5（非curatedで唯一puctが正）**、9/2/8ミラー +25.0★/+20.0・svb +12.5/+12.5 だが**対yamatoクロスで svb -12.5（2/7）・対h -12.5 と逆符号**
- ✅採用（コミット5b4001c・本番反映済み）: `PUCT_SKIP={'_OP14-020':1,'_OP16-022':1,'_OP10-099':1}`（heuristicフォールバック＝出荷比 +10〜+35pt級）／`PUCT_DEPTH` に `'_OP12-040'`・`'_OP16-079'` を PUCT_DEEP(9/2/8) で追加。**赤緑ルフィは保留**（現状の基準puctが全文脈で最良クラス。deepは追加帯・複数対面の再測定後に判断）。
- ★学び: ①**E53〜E59でheuristicを強化した結果、非curatedリーダーでは出荷「強いCPU」（浅puct）がheuristicより弱い逆転が起きていた**（mihawk -35pt★は本プロジェクト最大級の負値）＝heuristic改良の後は puct 側の per-leader 再監査を必ずセットにする ②ミラー2帯正でもクロス対面で逆符号があり得る（赤緑ルフィdeep）＝**採用バーに「実利用対面クロスの非退行」を追加** ③実対戦リプレイからのデッキ抽出（probe-decks.json方式）でプリセット外リーダーの測定が可能になった。
- 残: 赤緑ルフィdeepの追加測定／SKIP組への mcts 適合性（enel型の中間解）＝E61候補／モバイル実効値（cap後6/2/6）の確認はkuzan/yamatoともsvb正の6/2/6実測で代替済み。

## E61 / 2026-08-07: リプレイ研究第2弾由来の第1波7部品 → ✅3採用・❌2不採用・⏸2保留（+測定基盤バグ修正）

- 背景: `docs/pm/replay-study-20260807.md`（オンライン12戦・4並列解析）のS/A級候補から、実装仕様が明確な7部品を `E61_DEF`＋e61On（既定0=バイト挙動不変）で実装。攻撃系=finord（E55 orderの複合ゲート再設計）/koval（KO継続価値）、防御系=blockx（生存化ブロック）/guardcost（reko絞り版）/nowor（D.Atk前倒し止め）/lifefloor（Lmax下限）/wastecut（確実死温存）。回帰は unit-example 例33a〜g。
- 測定（heur2+OPCG_H2 単離・同一seedペア・N=120・5ライン=luffygb↔mihawk 2帯/teach→enel/yamato→kuzan/mihawk→luffygb）:
  - ✅**blockx**: 発火帯（luffygb視点 b1）で **+5.0pt（改善6/退行0 p=0.031★）**・他4線は中立（0〜1flip）。m21の「8000壁が6回ブロック無傷」の直移植。
  - ✅**finord**・✅**guardcost**: 全対面で無害（フリップ0〜2）＝発火機会がプリセット対面に乏しい。リプレイ実証（全12戦の詰め順序型・m16敗着2）を根拠に **E55の非発火6部品と同じ kohand型基準（勝率中立＋明確な無駄手削減）で採用**。
  - 合成（3部品）: **+5.8pt（改善7/退行0 p=0.016★）**/-0.8/-0.8 ＝単離を上回り負の干渉なし。
  - ❌**koval**: 負傾向（Σ改善8/退行18・-1.7/-1.7/-0.8/±0/-4.2）＝エンジンKO優先が対CPUではテンポ損。フラグ0温存。
  - ❌**wastecut**: **負符号再現（Σ改善0/退行7）**＝「確実死」判定がブロッカー無視の過大評価で、止まる列まで温存して落とす。
  - ⏸**nowor/lifefloor**: 全対面フリップ0＝プリセット対面に可視D.Atk持ちが不在で発火せず。実デッキ対面（probe-decks.json の赤緑ルフィ=OP13-118 D.Atk入り）を measure に載せる基盤を作ってから判定（E62候補）。
