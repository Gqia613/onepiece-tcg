    ...
    async function tryPlayHand(card) {
      ...
      if (b.type === 'CHAR') {
        ...
        flog(side, '「' + b.name + '」を使用'); cardReveal(side, b.no, b.name, 'イベント発動', 'event');
        P.trash.push(reset(card)); // ★発動時にトラッシュへ（公式Q&A1278/1279/94: トラッシュ枚数を数える効果は「このカード」を含める。トリガー発動は含めない）
        await runFx(b.fx.main.fx, { self: card, side }); render(); await luffyReveal(side); await fireOwnEventUsed(side); await fireOppEvent(side);
      }
    }
    ...
    async function heuristicTurn(side) {
      ...
      while (g++ < 8) {
        ...
        flog(side, '「' + c.base.name + '」を使用'); await fxNote(side, '効果使用', c.base.name);
        P.trash.push(reset(c)); // ★発動時にトラッシュへ（上と同じ公式裁定）
        await runFx(c.base.fx.main.fx, { self: c, side }); render(); await luffyReveal(side); await sleep(260); await fireOwnEventUsed(side); await fireOppEvent(side);
        if (G.winner) return;
      }
    ...
    async function randomTurn(side) {
      ...
      while (steps++ < 40 && !G.winner) {
        ...
        else if (p.k === 'event') { payDon(side, effCost(side, p.c)); P.hand.splice(P.hand.indexOf(p.c), 1); P.trash.push(reset(p.c)); /* ★発動時にトラッシュへ */ await runFx(p.c.base.fx.main.fx, { self: p.c, side }); await fireOwnEventUsed(side); await fireOppEvent(side); }
