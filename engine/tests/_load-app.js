...
function loadParts(htmlPath) {
  return {
    ...
    cardssets: fs.readFileSync(path.join(ROOT, 'cards-sets.js'), 'utf8'), // window.CARD_SETS（収録弾。番号の接頭辞≠弾）
...
function runHarness(name, harness, execOpts = {}) {
  const { stubs, cards, cardsfx, cardsattr, cardstrigger, cardssets, app } = loadParts();
  const tmp = path.join(os.tmpdir(), `opcg-${name}-${Date.now()}-${Math.random().toString(36).slice(2)}.js`);
  fs.writeFileSync(tmp, [stubs, cards, cardsfx, cardsattr, cardstrigger, cardssets, app, harness].join('\n'));
