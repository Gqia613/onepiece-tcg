...
export function ReplayBar() {
  ...
  const btn: React.CSSProperties = {
    minWidth: 32, height: 30, padding: '0 8px', borderRadius: 8, border: '1px solid var(--gold-dim)',
  ...
  const speedBtn = (s: 1 | 2 | 4) => (
    <button
      ...
      title={`再生速度 ${s}倍`}
      aria-label={`再生速度 ${s}倍`}
      style={{ ...btn, fontSize: 11.5, color: st.speed === s ? 'var(--gold-soft)' : 'var(--muted)', borderColor: st.speed === s ? 'var(--gold-soft)' : 'var(--line)' }}
  ...
  return (
    <div style={{
      ...
      maxWidth: 'calc(100vw - 12px)', flexWrap: 'wrap', justifyContent: 'center',
      background: 'linear-gradient(180deg, var(--ocean-800), var(--ocean-850))',
      border: '1px solid var(--gold-dim)', boxShadow: '0 10px 28px #000a',
    }}>
      <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--gold-soft)', whiteSpace: 'nowrap' }}>
      ...
      {!st.done ? (
        <>
          ...
          <button style={btn} title="結末までスキップ" aria-label="結末までスキップ" onClick={replaySkipToEnd}>⏭</button>
        </>
      ) : (
        <button style={btn} title="もう一度最初から見る" aria-label="もう一度最初から見る" onClick={replayRestart}>↻</button>
      )}
      <button
