  ...
  return (
    ...
    >
      ...
      <motion.div animate={fx} style={{ width: '100%', height: '100%', position: 'relative' }}>
        ...
        {/* 手札カードのコストバッジ（出せる=緑発光 / コスト軽減中=◆枠）。元 cardHTML の costChip。 */}
        {ctx === 'hand' && b.cost != null ? (
          <div className={'costchip' + (beh.highlight === 'playable' ? ' ok' : '') + (safeEffCost(engine, card) < (b.cost as number) ? ' disc' : '')}>
            {safeEffCost(engine, card)}
          </div>
        ) : null}
      </motion.div>
    </motion.div>
