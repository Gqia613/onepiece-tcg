    ...
    function effCost(side, c) { let cost = c.base.cost || 0; const m = c.base.costMod; if (m && checkCond(m.cond, side, G.players[side].leader)) cost += m.amount; const P = G.players[side]; const stg = P.stage; if (stg && !isNegated(stg) && stg.base.fx && stg.base.fx.static) for (const o of stg.base.fx.static) { if (o.op === 'playCostReduce' && G.active === side && (c.base.cost || 0) >= (o.minCost || 0) && matchFilter(c, o.filter || {})) cost -= o.amount || 0; } const tr = P._turnPlayCostReduce; if (tr && tr.turn === G.turnSeq && (c.base.cost || 0) >= (tr.minCost || 0) && matchFilter(c, tr.filter || {})) cost -= tr.amount || 0; for (const src of [P.leader, ...P.chars]) { if (!src || isNegated(src) || !src.base.fx || !src.base.fx.static) continue; for (const o of src.base.fx.static) { if (o.op === 'eventCostReduce' && matchFilter(c, o.filter || {})) cost -= o.amount || 0; } } return Math.max(0, cost); } // ステージ/ターン中/盤面提供の手札プレイコスト軽減（マリージョア/錦えもんL/OP01-067クロコ=青イベント-1）
