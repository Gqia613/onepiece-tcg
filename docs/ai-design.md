...
### 9.5 ⏭ さらに伸ばす（part3 候補）
1. **value も自己対戦で学習**（現在は手作り据え置き）＝puct分布の最終結果で2ヘッド化。`train.py` 多層・JS推論 `layers[]` 対応。
2. **探索を深く/多手先の木**（現状1手再探索）＋ **方策ターゲット=訪問数分布**（argmaxでなくsoft）。
3. **gating を全6リーダー**へ拡張、self-play 局数を増やす（replay buffer 拡大）。
- 限界（正直に）: 16GB/8コア/MPS単機は数百万局に届かない。狙いは**数万局規模の小型AlphaZero**。それでも part2 で「探索＋自己対戦＋gating」が **teach +16.3pt(有意)** を達成＝**正しい構造なら天井(≈heuristic)を超える**ことは実証済み。残りは規模と適用範囲の拡大。
