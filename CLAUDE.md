...
- ✅: タイミングフック → `onOppAttack`/`onTurnEnd`/`onAllyLeave`/`onReviveFromTrash`/**`onBlock`（【ブロック時】）** を追加済。onBlockは`declareAttack`のブロック宣言後・カウンター前に誘発（`{self:blocker,side:dSide,attacker}`）。15枚実装済（モネOP05-036(+_r1)/ヘルメッポOP12-033/ヒナOP02-110/戦桃丸EB04-053/ベラミーOP10-077/ジンベエOP01-014/キラーOP01-039(+_r1)/ハンコックOP01-078(+_r1・onAttack兼)/ウタST05-004(+_r1)/ブラックマリアOP01-111/ホーキンスOP05-047）。複合条件は`{and:[...]}`＋既存obj cond(`selfCharCount`/`selfHandAtMost`等)で表現。「このキャラは…+N」は**`powerMod target:'self'`**（ctx.self自身にaddBuff・新設）。**未実装3枚**: シュライヤOP06-009(=【ターン1回】once:'turn'をonBlock/onAttackに要追加＋setPower self/oppLeaderPower)・しのぶST09-007(=ライフコスト→自身+4000)・クロコダイルST03-003(=「コスト2以下のキャラ」の対象が自他どちらも取れる曖昧文。deckBottom opは相手専用)。
