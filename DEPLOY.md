...
functions が参照する env: `DB`(D1) / `JWT_SECRET` / `ANTHROPIC_API_KEY` / `ANTHROPIC_VERSION` / `AICACHE`(KV)。
...
- **Settings → Environment variables (Production) → Secret**: `JWT_SECRET`（＋必要なら `ANTHROPIC_API_KEY`）を追加
